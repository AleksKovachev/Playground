...
class SnakesAndLadders:
    ...
    def __init__(self):
        ...
        # Player name input
        self.player1 = input(f'Please {_SB}Enter{_SR} the name of the first player: ')
        self.player2 = input(f'Please {_SB}Enter{_SR} the name of the second player: ')
    ...
    def start_game(self):
        ...
        self.player1_color = input(f"{_SB}{self.player1}{_SR}, choose a color : " \
            f"1 = {_FR}Red{FR_}, 2 = {_FB}Blue{FR_}, 3 = {_FG}Green{FR_}, 4 = {_FY}Yellow{FR_}\n")

        self.player_color(self.player1_color, self.player1)

        self.player2_color = input(f"{_SB}{self.player2}{_SR}, choose a color " \
            f": 1 = {_FR}Red{FR_}, 2 = {_FB}Blue{FR_}, 3 = {_FG}Green{FR_}, 4 = {_FY}Yellow{FR_}\n")
    ...
    def player_color(self, input_color, player):
        ...
        if player == self.player2 and input_color == self.player1_color:
            print(f"{_SB}{self.player1}{_SR} already took that color!")
            input_color = input(f"{_SB}{player}{_SR}, choose a color : 1 = {_FR}Red{FR_}, " \
                f"2 = {_FB}Blue{FR_}, 3 = {_FG}Green{FR_}, 4 = {_FY}Yellow{FR_}\n")
        ...
        # Pick the color by integer
            ...
            print(f"{getattr(Fore, color)}{player} is now {color.lower()}!{FR_}")

            if player == self.player1:
                self.player1 = getattr(Fore, color) + player + FR_
            elif player == self.player2:
                self.player2 = getattr(Fore, color) + player + FR_

        else:
            print(f"There's no color assigned to {_SB}{input_color}{_SR}, " \
                f"enter a number between {_SB}1{_SR} and {_SB}4{_SR}")
            input_color = input(f"{_SB}{player}{_SR}, choose a color : 1 = {_FR}Red{FR_}, " \
                f"2 = {_FB}Blue{FR_}, 3 = {_FG}Green{FR_}, 4 = {_FY}Yellow{FR_}\n")
            self.player_color(input_color, player)
    ...
    def first_roll(self):
        ...
        pl1 = input(f"{self.player1} press {_SB}Enter{_SR} to roll the dice" \
            f"or {_SB}x{_SR} to quit the game ")
    ...
    def pl_pos_check(self, roll, pos, player):
        ...
        if pos[0] < 100:
            print(f"{player}'s position is : {_SB}{pos[0]}{_SR}\n")
        ...
        # If position exceeds 100 then player stays at the same position
            ...
            pos[0] -= roll
            print(f"{player}'s position {_SB}remains : {pos[0]}{_SR}\n")
            return pos

        # If position is 100 - Congradulates the winner and asks if a new game should be started
        print(
            f"{_SB}{player}, you WIN!!! Congrats!{_SR}\nDo you want to start a {_SB}new game?{_SR}"
            )
        self.new_game()
    ...
    def game_roll(self, curr_player, other_player, curr_pos, other_pos):
        ...
        input_ = input(f"{'-'*50}\n{curr_player} press {_SB}Enter{_SR} to roll the dice ")
        ...
        print(f"{curr_player} rolled : {Fore.MAGENTA}{_SB}{roll}{_SR}")
        curr_pos[0] += roll
        ...
        if self.attacks is True and curr_pos == other_pos:
            print(f"Sorry {other_player}, {curr_player} {_SB}stabbed{_SR} you from behind! " \
    ...
    def new_game(self):
        ...
        rematch = input("Do you want to play another game?\n" \
            f"Type {_SB}Yes{_SR} to start a new game and {_SB}No{_SR} to quit game: ")
        ...
        # Announce the winner and quit!
            ...
            if self.p1_pos[0] > self.p2_pos[0]:
                print(f"{_SB}Congrats to {self.player1} for the win!{_SR}")

            elif self.p2_pos[0] > self.p1_pos[0]:
                print(f"{_SB}Congrats to {self.player2} for the win!{_SR}")

            else:
                print(f"{_SB}It's a draw!{_SR}")

            print("Goodbye!")
            sys.exit()

        else:
            print(f"You must Enter {_SB}Yes{_SR} or {_SB}No{_SR}!")
            self.new_game()
...
def stabs():
    ...
    stabbing = input(
        f"Do you want {_FC}{_SB}stabbing{_SR} to be enabled?\n" \
            f"Type '{_FC}{_SB}Yes{_SR}' or '{_FC}{_SB}No{_SR}' or type " \
                f"'{_FC}{_SB}Info{_SR}' for more info on 'stabbing' "\
                    f"and '{_FC}{_SB}x{_SR}' to exit the game\n\n")

    # If 'Yes' was typed - stabbing is turned On
    if stabbing.lower() in ("yes", "y", "1"):
        print(f"Stabbing is {_FC}{_SB}On{_SR}")

    # If 'No' was typed - stabbing is turned Off
    elif stabbing.lower() in ("no", "n", "0"):
        SnakesAndLadders.attacks = False
        print(f"Stabbing is {_FC}{_SB}Off{_SR}")
    ...
    # If anything else was typed - show the options and restart the question
        ...
        print(f"You must enter: '{_FC}{_SB}Yes{_SR}', '{_FC}{_SB}No{_SR}', " \
            f"'{_FC}{_SB}Info{_SR}' or '{_FC}{_SB}x{_SR}'")
        stabs()
...
def board(pos, player):
    ...
    if pos[0] in LADDERS:
        print(f"{player}, you encountered a {_FB}{_SB}ladder{_SR}! Congrats!\n" \
            f"Your position changes from {pos[0]} to {LADDERS[pos[0]]}")
        pos[0] = LADDERS[pos[0]]

        print(f"{player}'s position is : {_FB}{_SB}{pos[0]}{_SR}\n")
        return pos

    if pos[0] in SNAKES:
        print(f"{player}, you encountered a {_FR}{_SB}snake{_SR}! Sorry!\n" \
            f"Your position changes from {pos[0]} to {SNAKES[pos[0]]}")
        pos[0] = SNAKES[pos[0]]

        print(f"{player}'s position is : {_FR}{_SB}{pos[0]}{_SR}\n")
        return pos
